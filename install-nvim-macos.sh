#!/usr/bin/env bash
# ============================================================================
# Neovim One-Click Installer — macOS
# Installs Neovim + all plugins, LSP servers, formatters, and config files.
# Usage:  curl -fsSL <gist_raw_url> | bash
#    or:  chmod +x install-nvim-macos.sh && ./install-nvim-macos.sh
# ============================================================================
set -euo pipefail

echo "╔══════════════════════════════════════════════╗"
echo "║  Neovim Installer — macOS                    ║"
echo "╚══════════════════════════════════════════════╝"
...
brew install neovim ripgrep fzf fd node python3 cmake make gcc || true

# Ensure Python/Node providers are available
pip3 install --user --break-system-packages pynvim 2>/dev/null || pip3 install --user pynvim || true
npm install -g neovim || true

# Formatters & linters used by config
pip3 install --user --break-system-packages black 2>/dev/null || pip3 install --user black || true
brew install stylua shfmt || true
...
echo "▸ Writing Neovim config to $NVIM_DIR …"
...
# ── lua/settings.lua ──────────────────────────────────────────────────────
cat > "$NVIM_DIR/lua/settings.lua" << 'SETTINGSEOF'
...
-- Mouse support (all modes) — fixes clicks/scrollbar in split panes
opt.mouse = "a"
...
# ── lua/keymaps.lua ───────────────────────────────────────────────────────
cat > "$NVIM_DIR/lua/keymaps.lua" << 'KEYMAPSEOF'
...
# ── lua/plugins.lua ───────────────────────────────────────────────────────
cat > "$NVIM_DIR/lua/plugins.lua" << 'PLUGINSEOF'
return {
  -- ==========================================
  -- Theme
  -- ==========================================
...
# ── Bootstrap plugins ─────────────────────────────────────────────────────
echo "▸ Bootstrapping plugins (lazy.nvim will clone everything)…"
nvim --headless "+Lazy! sync" +qa 2>/dev/null || true

echo ""
echo "╔══════════════════════════════════════════════╗"
echo "║  ✅ Done! Launch nvim to enjoy your setup.   ║"
echo "║  Tip: Set your terminal font to              ║"
echo "║       JetBrainsMono Nerd Font for icons.     ║"
echo "╚══════════════════════════════════════════════╝"
